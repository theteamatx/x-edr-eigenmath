...
#ifndef EIGENMATH_EIGENMATH_SO3_H_
#define EIGENMATH_EIGENMATH_SO3_H_
...
// Helper class for SO3 to check if a quaternion is normalized and generate
// an error string on the stack if not.
// As this is also used in real-time code, it is essential that it doesn't
// allocate heap memory.
// This class is an implementation detail of SO3 below.
class SO3NormalizationChecker {
 public:
  SO3NormalizationChecker() = delete;
  SO3NormalizationChecker(const SO3NormalizationChecker&) = delete;

  template <typename Scalar, int Options>
  EIGEN_DEVICE_FUNC explicit SO3NormalizationChecker(
      const Quaternion<Scalar, Options>& quaternion) {
    using std::abs;  // for ADL
    is_normalized_ = abs(quaternion.squaredNorm() - Scalar(1)) <
                     Eigen::NumTraits<Scalar>::dummy_precision();
    if (is_normalized_) {
      return;
    }
  }
  // Returns true is the quaternion *this was constructed with is normalized.
  EIGEN_DEVICE_FUNC bool IsNormalized() const { return is_normalized_; }
...
class SO3 {
 public:
  ...
  // Initializes with angle/axis
  //
  // Angle and axis pair as a user representation of orientation.
  SO3(Scalar angle, const Vector3<Scalar>& axis)
      : quaternion_(Eigen::AngleAxis<Scalar>(angle, axis)) {}
  ...
  EIGEN_DEVICE_FUNC explicit SO3(const Quaternion<OtherOptions>& quaternion,
                                 bool do_normalize = true)
      : quaternion_(quaternion) {
    if (do_normalize) {
      quaternion_.normalize();
    }
    const internal::SO3NormalizationChecker checker(quaternion_);
    CHECK(checker.IsNormalized());
  }
...
template <typename Scalar, int Options>
void RotationToRPY(const SO3<Scalar, Options>& s, Scalar* roll, Scalar* pitch,
                   Scalar* yaw) {
...
#endif  // EIGENMATH_EIGENMATH_SO3_H_
